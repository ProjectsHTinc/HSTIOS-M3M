...
//            deleteDB(dbURL: dbURL)
    ...
    func createTables() throws {
        ...
        let ret =  sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS Create_location_store_data (id INTEGER UNIQUE PRIMARY KEY AUTOINCREMENT, user_id TEXT NOT NULL, lat TEXT UNIQUE NOT NULL, lon TEXT NOT NULL, location TEXT NOT NULL, dateandtime TEXT NOT NULL, distance TEXT NOT NULL, pia_id TEXT UNIQUE NOT NULL, gps_status TEXT NOT NULL, server_id TEXT UNIQUE NOT NULL, sync_status TEXT NOT NULL)", nil, nil, nil)
        if (ret != SQLITE_OK) { // corrupt database.
            logDbErr("Error creating db table - Create_location_store_data")
